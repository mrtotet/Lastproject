...
class LoginForm(Html5Mixin, forms.Form):
    ...
    Fields for login.
    """
    username = forms.CharField(label=username_label, widget=forms.TextInput(attrs={'placeholder': 'username/email'}))
    ...
    def clean(self):
        ...
        if self._user is None:
            raise forms.ValidationError(
                             ugettext("username/email và password không tồn tại"))
        elif not self._user.is_active:
            raise forms.ValidationError(ugettext("Tài khoản chưa được kích hoạt"))
...
class ProfileForm(Html5Mixin, forms.ModelForm):
    ...
    """
    phone = forms.CharField(label=_("Số điện thoại"), )
    date_of_birth = forms.DateField(label=_("Ngày tháng năm sinh"),required=False  )
    avatar = forms.ImageField(label=_("Upload Ảnh đại diện"),required=False  )
    contact_address = forms.CharField(label=_("Địa chỉ liên hệ"), max_length=150,required=False )

    password1 = forms.CharField(label=_("Password"),
                                widget=forms.PasswordInput(render_value=False),required=False)
    password2 = forms.CharField(label=_("Nhập lại Password"),
                                widget=forms.PasswordInput(render_value=False),required=False)
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            self.fields["username"].help_text = ugettext(
                        "username chỉ bao gồm chữ số và các kí tự '-','_'")
        except KeyError:
            pass
    ...
    def clean_username(self):
        ...
        if username.lower() != slugify(username).lower():
            raise forms.ValidationError(
                ugettext("username chỉ bao gồm chữ số và "
                         "và các kí tự '-','_' "))
    ...
    def clean_password2(self):
        ...
        if password1:
            ...
            if password1 != password2:
                errors.append(ugettext("Passwords lặp lại không khớp"))
            if len(password1) < settings.ACCOUNTS_MIN_PASSWORD_LENGTH:
                errors.append(
                        ugettext("Password phải ít nhất %s ký tự") %
    ...
    def clean_email(self):
        ...
        if len(qs) == 0:
            return email
        raise forms.ValidationError(
                                ugettext("Email đã được sử dụng"))
...
class Signup_Form(Html5Mixin, forms.ModelForm):
    """
    ModelForm for auth.User - used for signup and profile update.
    If a Profile model is defined via ``ACCOUNTS_PROFILE_MODEL``, its
    fields are injected into the form.
    """
    phone = forms.CharField(label=_("Số điện hoại"), )
    password1 = forms.CharField(label=_("Password"),
                                widget=forms.PasswordInput(render_value=False))
    password2 = forms.CharField(label=_("Nhập lại Password"),
                                widget=forms.PasswordInput(render_value=False))
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['password2'].widget.attrs.update({'placeholder': 'nhập lại password'})
        self.fields['first_name'].widget.attrs.update({'placeholder': 'họ và tên'})
        self.fields['email'].widget.attrs.update({'placeholder': 'email'})
        self.fields['username'].widget.attrs.update({'placeholder': 'username'})
        self.fields['phone'].widget.attrs.update({'placeholder': 'số điện thoại'})
        self._signup = self.instance.id is None
        user_fields = set([f.name for f in User._meta.get_fields()])
        try:
            self.fields["username"].help_text = ugettext(
    ...
    def save(self, *args, **kwargs):
        ...
        kwargs["commit"] = False
        user = super(Signup_Form, self).save(*args, **kwargs)
        try:
            self.cleaned_data["username"]
        except KeyError:
            if not self.instance.username:
